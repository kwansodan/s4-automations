          ...
          {isAccountingFirm ? (
            ...
            <div className="space-y-3">
              <div className="p-3 bg-slate-950/80 border border-slate-800 rounded-xl space-y-2">
                <div className="flex items-center gap-2.5">
                  ...
                  <div className="min-w-0">
                    <p className="text-xs font-bold text-white truncate">{currentClient?.name}</p>
                    <p className="text-[10px] text-amber-400 font-mono">Direct Business Workspace</p>
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-2 pt-2 border-t border-slate-900 text-[11px]">
                  <div className="bg-slate-900/80 p-2 rounded-lg">
                    ...
                    <span className="font-bold text-white font-mono">{currentClient?.pipelines?.length || 0} active</span>
                  </div>
                  <div className="bg-slate-900/80 p-2 rounded-lg">
                    <span className="text-slate-400 text-[9px] block uppercase font-mono">Platform</span>
                    <span className="font-bold text-emerald-400 capitalize">{currentClient?.accounting_software?.replace('_', ' ') || 'Zoho'}</span>
                  </div>
                </div>
              </div>
                  ...
                  {user.organizations
                    ...
                    .map((org) => (
                      <button
                        ...
                        className="w-full flex items-center justify-between p-2 rounded-lg bg-slate-950/60 hover:bg-slate-800 border border-slate-800 text-left transition cursor-pointer group"
                      >
                        <div className="flex items-center gap-2 min-w-0">
                          <span className="text-base shrink-0">{org.icon || '🏛️'}</span>
                          <div className="min-w-0">
                            <p className="text-xs font-semibold text-white group-hover:text-sky-300 truncate">{org.name}</p>
                            <p className="text-[9px] text-slate-400 font-mono">
                              {org.org_type === 'ACCOUNTING_FIRM' ? 'Accounting Firm Practice' : 'Direct Business'}
                            </p>
                          </div>
                        </div>
                        <ArrowRightLeft className="w-3.5 h-3.5 text-slate-400 group-hover:text-sky-400 shrink-0" />
                      </button>
                    ))}
                </div>
              )}
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-1.5 text-xs font-bold text-white bg-gradient-to-r from-amber-600 to-orange-600 hover:from-amber-500 hover:to-orange-500 py-2 rounded-lg shadow-md transition cursor-pointer"
              >
