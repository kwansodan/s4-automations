...
export const Sidebar: React.FC<SidebarProps> = ({ isMobileOpen, setIsMobileOpen }) => {
  ...
  return (
    <>
      ...
      {/* Sidebar Container */}
      <aside
        className={`fixed top-0 left-0 bottom-0 z-50 flex flex-col bg-white border-r border-[#E2E8F0] transition-all duration-300 shadow-[1px_0_3px_0_rgba(15,23,42,0.02)] ${
      ...
      >
        ...
        <div className="h-16 flex items-center justify-between px-3.5 border-b border-[#E2E8F0] shrink-0 relative">
          <div
            onClick={() => handleNav('workspace', 'overview')}
            className="flex items-center gap-2.5 cursor-pointer group overflow-hidden min-w-0"
          >
            <div className={`w-9 h-9 rounded-xl flex items-center justify-center text-white shadow-sm shrink-0 group-hover:scale-105 transition-transform ${
              isIndividualBusiness
                ? 'bg-gradient-to-tr from-[#0284C7] to-[#0EA5E9] shadow-[#0284C7]/20'
                : 'bg-gradient-to-tr from-[#0284C7] to-[#38BDF8] shadow-[#0284C7]/25'
            }`}>
              {isIndividualBusiness ? (
            ...
            {!isCollapsed && (
              <div className="min-w-0">
                <span className="text-xs font-bold text-slate-900 tracking-tight block truncate group-hover:text-[#0284C7] transition-colors">
                  {isIndividualBusiness
                    ? (activeOrganization?.name?.split('(')[0].trim() || 'ANR Group')
                    : 'S4 Automations'}
                </span>
                <span className="text-[10px] font-mono block truncate text-slate-500">
          ...
          <button
            ...
            className="hidden lg:flex p-1 rounded-lg text-slate-400 hover:text-slate-700 hover:bg-slate-100 border border-transparent transition cursor-pointer"
        ...
        {!isCollapsed && user?.organizations && user.organizations.length > 1 && (
          <div className="px-3 pt-2.5 relative">
            <button
              ...
              className="w-full text-left flex items-center justify-between p-2 rounded-xl bg-slate-50 hover:bg-slate-100 border border-[#E2E8F0] hover:border-sky-300 transition cursor-pointer group"
            >
              <div className="flex items-center gap-2 min-w-0">
                <span className="text-sm shrink-0">{activeOrganization?.icon || (isIndividualBusiness ? '🧺' : '🏛️')}</span>
                <div className="min-w-0">
                  <span className="text-[11px] font-bold text-slate-900 block truncate group-hover:text-[#0284C7]">
              ...
              <ChevronDown className={`w-3 h-3 text-slate-400 group-hover:text-slate-600 transition-transform ${isOrgDropdownOpen ? 'rotate-180' : ''}`} />
            </button>

            {isOrgDropdownOpen && (
              <div className="absolute left-3 right-3 top-full mt-1 bg-white border border-[#E2E8F0] rounded-xl p-1.5 shadow-xl z-50 animate-in fade-in">
                  ...
                  return (
                    <button
                      ...
                      className={`w-full text-left flex items-center justify-between gap-2 px-2.5 py-1.5 rounded-lg text-xs transition cursor-pointer ${
                        isCurrent ? 'bg-[#F0F9FF] text-[#0284C7] font-bold border border-[#BAE6FD]' : 'text-slate-700 hover:bg-slate-50 hover:text-slate-900'
                      }`}
                    >
                      <div className="flex items-center gap-2 min-w-0">
                        ...
                        <div className="min-w-0">
                          ...
                          <span className={`text-[8px] font-mono block ${isCurrent ? 'text-[#0284C7]' : 'text-slate-400'}`}>
                            {org.org_type === 'INDIVIDUAL_BUSINESS' ? 'Direct Business' : 'Accounting Practice'}
                          </span>
                        </div>
                      </div>
                      {isCurrent && <Check className="w-3.5 h-3.5 shrink-0 text-[#0284C7]" />}
                    </button>
                  );
                })}
              </div>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar py-4 px-3 space-y-6">
          ...
          <div className="space-y-1">
            {!isCollapsed && (
              <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400 px-3 block mb-1.5 font-mono">
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-3 py-2 rounded-xl text-xs font-semibold transition-all cursor-pointer ${
                    isActive
                      ? 'bg-[#F0F9FF] text-[#0284C7] border border-[#BAE6FD] font-bold shadow-xs'
                      : 'text-slate-600 hover:text-slate-900 hover:bg-slate-50 border border-transparent'
                  }`}
                >
                  <Icon className={`w-4 h-4 shrink-0 ${isActive ? 'text-[#0284C7]' : 'text-slate-400'}`} />
                  {!isCollapsed && (
                    <div className="flex items-center justify-between flex-1 min-w-0">
                      <span className="truncate">{item.label}</span>
                      {item.badge && (
                        <span className={`text-[10px] font-mono font-bold px-1.5 py-0.5 rounded ${
                          isActive ? 'bg-[#0284C7] text-white' : 'bg-[#F0F9FF] text-[#0284C7] border border-[#BAE6FD]'
                        }`}>
                          {item.badge}
                        </span>
                      )}
                ...
                >
                  ...
                  {!isCollapsed && (
                    <div className="flex items-center justify-between flex-1 min-w-0">
                      ...
                      {item.badge && (
                        <span className={`text-[10px] font-mono font-bold px-1.5 py-0.5 rounded ${
                          isActive ? 'bg-[#0284C7] text-white' : 'bg-slate-100 text-slate-600 border border-slate-200'
                        }`}>
                          {item.badge}
                        </span>
                      )}
        ...
        <div className="p-3 border-t border-[#E2E8F0] shrink-0 space-y-2 bg-white">

          {/* User Profile / Logout */}
          <div className="flex items-center justify-between gap-2 p-1.5">
            <div className="flex items-center gap-2 min-w-0">
              <span className="w-8 h-8 rounded-xl bg-slate-100 border border-slate-200 flex items-center justify-center text-xs text-[#0284C7] font-bold shrink-0">
              ...
              {!isCollapsed && (
                <div className="min-w-0">
                  ...
                  <span className="text-[10px] text-[#059669] font-mono block">
            ...
            <button
              onClick={() => logout()}
              title="Sign Out"
              className="p-1.5 rounded-lg text-slate-400 hover:text-[#E11D48] hover:bg-[#FFF1F2] transition cursor-pointer"
            >
